...
export default function ProfesorMicroPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    let payload: any = {
      ...
      id_equipoMicro: equipoSeleccionado,
    };
    ...
    if (tipoProfesor === "REGISTRADO") {
      ...
      payload.profesor = { id_profMicro: profesorSeleccionado };
    }
    ...
    if (data.ok) {
      let profesorNombre = '';
      if (tipoProfesor === "REGISTRADO") {
        const prof = profesores.find(p => p.id_profMicro === Number(profesorSeleccionado));
        if (!prof) {
      ...
      const equipo = equipos.find(eq => eq.id_equipoMicro === Number(equipoSeleccionado));
      if (!equipo) {
        alert("No se encontró el equipo seleccionado.");
        return;
      }
      const equipoNombre = equipo.nombre_proyecto || '';
      const idProfMicro = data.id_profMicro;
      const idEquipoMicro = equipoSeleccionado;
      router.push(`/micro_rubrica?id_equipoMicro=${idEquipoMicro}&id_profMicro=${idProfMicro}&profesor_nombre=${encodeURIComponent(profesorNombre)}&equipo_nombre=${encodeURIComponent(equipoNombre)}`);
    }
  };
      ...
      <div style={estilos.contenedor}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            <option value="">SELECCIONE...</option>
            {equipos.map((eq) => (
              <option key={eq.id_equipoMicro} value={eq.id_equipoMicro}>
                {eq.nombre_proyecto}
              </option>
            ))}
          </select>
