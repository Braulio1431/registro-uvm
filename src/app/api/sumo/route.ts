...
export async function GET() {
  try {
    ...
    const [categoria]: any = await db.execute(
      "SELECT id_categoria FROM categorias_competencia WHERE LOWER(nombre_categoria) = 'sumo'"
    );

    if (!categoria || categoria.length === 0) {
      return NextResponse.json(
        { error: "Categoría SUMO no encontrada" },
        { status: 404 }
      );
    }
    ...
    console.error("Error en /api/sumo:", error);
    return NextResponse.json(
